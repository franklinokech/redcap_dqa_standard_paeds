...
get_project_root <- function() {
  ...
  while (!file.exists(file.path(current_dir, ".git")) &&
         ...
         !file.exists(file.path(current_dir, "cin_paeds"))) {
...
get_config <- function() {
  ...
  list(
    ...
    db_path = Sys.getenv("DUCKDB_PATH", 
      file.path(project_root, "cin_paeds", "data", "processed", "paeds.duckdb")),
    output_dir = Sys.getenv("DQA_OUTPUT_DIR",
      file.path(project_root, "cin_paeds", "data", "output")),
    project_name = Sys.getenv("PROJECT_NAME", "cin_paeds"),
    sql_file = Sys.getenv("DQA_SQL_FILE",
      file.path(project_root, "cin_paeds", "scripts", "sql", "dqa_queries.sql"))
  )
}
...
run_dqa_queries <- function() {
  ...
  cat(strrep("=", 60), "\n")
  cat("CIN Paediatrics - DQA QUERY PIPELINE\n")
